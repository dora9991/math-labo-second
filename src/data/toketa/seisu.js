...
function genDaisho() {
  ...
  if (r < 0.34) {
    ...
    return { q: `0 より ${n} ${big ? "大きい" : "小さい"}数を、符号をつけて答えよう`, ans,
      steps: ["0より大きい→＋、0より小さい→−", `0より ${n} ${big ? "大きい" : "小さい"} → ${nm(ans)}`],
...
function genKahou() {
  ...
  const steps = same
    ? ["同符号（＋どうし／−どうし）", `絶対値をたす：${A}＋${Bb}＝${A + Bb}`, `符号はそのまま ${a < 0 ? "−" : "＋"}`]
    : ["異符号（＋と−）", `絶対値の大きい方から小さい方をひく：${Math.max(A, Bb)}−${Math.min(A, Bb)}＝${Math.abs(ans)}`, `符号は絶対値が大きい方（${(A > Bb ? a : b) < 0 ? "−" : "＋"}）`];
  const wrongs = same
...
function genGenpou() {
  ...
  return { q: `${sp(a)} − ${sp(b)}`, ans, viz: { start: a, delta: -b, ans },
    steps: ["ひき算は、ひく数の符号を変えて「たす」に直す", `${sp(a)} ＋ ${sp(-b)} になる`, `＝ ${ans}`],
...
function genChain() {
  ...
  return { q, ans, steps: ["ひき算は「符号を変えてたす」に直し、前から順に計算", `${calcLine.join(" ")} ＝ ${ans}`],
...
function genShisoku() {
  if (Math.random() < 0.55) {
    ...
    return { q: `${sp(a)} ＋ ${sp(b)} × ${c}`, ans,
      steps: ["×・÷ を先に計算する", `${sp(b)} × ${c} ＝ ${prod}`, `あとは加法：${nm(a)} ＋ ${nm(prod)} ＝ ${ans}`],
  ...
  return { q: `${sp(sa * a)} × ${b} − ${c} ÷ ${sp(sd * d)}`, ans,
    steps: ["×・÷ を先に計算する", `${sp(sa * a)}×${b}＝${t1}`, `${c}÷${sp(sd * d)}＝${t2}`, `${nm(t1)} − (${nm(t2)}) ＝ ${ans}`],
